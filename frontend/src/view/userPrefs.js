...
export default function UserPrefs() {
    ...
    let allStates = [museum, cultural, religious, nature, indoors, thrill, avoidTourists, walkable, restaurant]
    let preferencesList = []
    ...
    useEffect(() => {
        ...
        if (restaurant) {
            preferencesList.push(categories["restaurant"])
        }

        preferencesList = preferencesList.flat()
        console.log(preferencesList)
        // setPreferencesList(preferencesList);
    }, [museum, cultural, religious, nature, indoors, thrill, avoidTourists, walkable, restaurant])
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (data.prev.includes("restaurant")) {
                setRestaurant(1)
            }
        }
    ...
    return (
        <>  
            ...
            <UserPrefCheckbox label={"Restaurant"} state={restaurant} setState={setRestaurant}></UserPrefCheckbox>
            <br></br>
