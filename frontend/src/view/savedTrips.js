...
export default function SavedTripsDynamic() {
    ...
    const handleDeleteTrip = async (index) => {
        // console.log('Deleting trip with index:', index);
        const updatedTrips = [...allTrips];
        const tripToDelete = updatedTrips[index];

        const response = await fetch('http://localhost:3001/deleteTrip', {
            method: 'DELETE',
            headers: {
            ...
            body: JSON.stringify({
              ...
              tripId: tripToDelete.id, // Assuming each trip has an 'id' property
            }),
          });
    ...
    if (data.success) {
      ...
      // Optionally display a message that the trip has been deleted
                        ...
                        <div className="card" key={index} id={index}>
                            <div className="card-header bold btn-link btn border-0" id="headingOne">
                                <h2 className="mb-0">
                                    ...
                                    {/* <button className="btn btn-danger ml-2" onClick={() => handleDeleteTrip(index)}>Delete</button> */}
