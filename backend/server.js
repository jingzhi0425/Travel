...
const mongoAtlasUri = "mongodb+srv://yeowjingzhi:DVW8uHDdHPyLsaQ2@cluster0.lb8jid7.mongodb.net/travel?retryWrites=true&w=majority";
...
app.delete('/deleteTrip', async (req, res) => {
  try {
    ...
    const userTrips = await Routes.findOne({ email: email });

    if (userTrips) {
      const updatedTrips = userTrips.trips.filter(trip => trip._id != tripId);
      userTrips.trips = updatedTrips;
      await userTrips.save();
      
      res.status(200).json({ message: 'Trip deleted successfully', updatedTrips });
    } else {
...
app.put('/updateTrip', async (req, res) => {
  try {
    const { email, tripId } = req.params;
    const { placeName, startDate, endDate, routes } = req.body;

    const userTrips = await Routes.findOne({ email: email });

    if (userTrips) {
      const tripToUpdate = userTrips.trips.find(trip => trip._id == tripId);

      if (tripToUpdate) {
        tripToUpdate.placeName = placeName || tripToUpdate.placeName;
        tripToUpdate.startDate = startDate || tripToUpdate.startDate;
        tripToUpdate.endDate = endDate || tripToUpdate.endDate;
        tripToUpdate.routes = routes || tripToUpdate.routes;

        await userTrips.save();
        
        res.status(200).json({ message: 'Trip updated successfully', updatedTrip: tripToUpdate });
      } else {
